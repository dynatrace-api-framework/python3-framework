...
def generate_mz_payload(application, env_zone=None):
  ...
  mz_payload['rules'][0]['conditions'][0]['comparisonInfo']['value']['value'] = str(application)
  mz_payload['rules'][1]['conditions'][0]['comparisonInfo']['value']['value'] = str(application)

  for rule_num in range(2, 10):
    mz_payload['rules'][rule_num]['conditions'][0]['comparisonInfo']['value']['key'] = "APP: " + str(application)
  ...
  if env_zone:
    ...
    # Service and Process Groups are different because they allow Key/Value Pairs
    condition_payload = copy.deepcopy(mz_payload['rules'][0]['conditions'][0])
    ...
    condition_payload = copy.deepcopy(mz_payload['rules'][1]['conditions'][0])
    condition_payload['comparisonInfo']['value']['key'] = "ENV"
    condition_payload['comparisonInfo']['value']['value'] = str(env_zone)
    ...
    for rule_num in range(2, 10):
      del condition_payload
      condition_payload = copy.deepcopy(mz_payload['rules'][rule_num]['conditions'][0])
      condition_payload['comparisonInfo']['value']['key'] = "ENV: " + str(env_zone)
      mz_payload['rules'][rule_num]['conditions'].append(condition_payload)
...
def change_management_zone(cluster, tenant, mz_id, application, env_zone=None):
  ...
  response = rh.config_put(
      cluster,
      tenant,
      'managementZones/' + str(mz_id),
      json=mz_payload
  )
